...
const SellerProducts: React.FC = () => {
  ...
  const deleteImageMutation = useMutation({
    mutationFn: ({ productId, imagePath }: { productId: string; imagePath: string }) =>
      sellerApi.deleteProductImage(productId, imagePath),
                ...
                <tbody>
                  {filtered.map((p: Product, idx: number) => (
                    <tr key={p.id} className={`border-b border-gray-100 dark:border-gray-800 last:border-0 ${idx % 2 === 0 ? '' : 'bg-gray-50/40 dark:bg-gray-800/20'}`}>
                      ...
                      <td className="px-4 py-3">
                        <StatusBadge status={p.isActive ? 'active' : 'inactive'} customLabel={p.isActive ? 'Active' : 'Inactive'} />
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-2">
                          <button
                            onClick={() => openEdit(p)}
                            className="text-gray-400 dark:text-gray-500 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors p-1 rounded hover:bg-emerald-50 dark:hover:bg-emerald-900/20"
                          >
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              {newImagePreviews.length > 0 && (
                <p className="text-[10px] text-emerald-600">{newImagePreviews.length} new image{newImagePreviews.length !== 1 ? 's' : ''} ready to upload</p>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                ...
                <Select value={form.gender || 'none'} onValueChange={(v) => setFormField('gender', v === 'none' ? '' : v)}>
              ...
              <div className="space-y-1">
                ...
                <Select value={form.wearType || 'none'} onValueChange={(v) => setFormField('wearType', v === 'none' ? '' : v)} disabled={!form.gender}>
              ...
              <div className="space-y-1">
                ...
                <Select value={form.category || 'none'} onValueChange={(v) => setFormField('category', v === 'none' ? '' : v)} disabled={!form.wearType}>
              ...
              <div className="space-y-1">
                ...
                <Select value={form.subCategory || 'none'} onValueChange={(v) => setFormField('subCategory', v === 'none' ? '' : v)} disabled={subCatsForCategory.length === 0}>
