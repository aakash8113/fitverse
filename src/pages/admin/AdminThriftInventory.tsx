...
interface ThriftProductFormData {
  name: string;
  description: string;
  price: string;
...
const AdminThriftInventory: React.FC = () => {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['admin', 'thrift-inventory', search, genderFilter, page],
    queryFn: () =>
  ...
  const getErrorDescription = (e: any): string => {
    ...
    if (errData?.errors && Array.isArray(errData.errors) && errData.errors.length > 0) {
      return errData.errors.map((err: { field: string; message: string }) => `${err.field}: ${err.message}`).join('\n');
    }
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: 'Thrift product created' });
      setDialogOpen(false);
    },
    ...
    <AdminLayout>
      <div className="p-6 space-y-5">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Thrift Inventory</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Manage thrift products listed in the store</p>
          </div>
          <Button size="sm" className="gap-1.5" onClick={openCreate}>
            <Plus className="h-4 w-4" /> Add Thrift Product
          </Button>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1 max-w-xs">
            ...
            <Input
              placeholder="Search thrift products..."
              value={search}
        ...
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
          {isLoading ? (
            ...
            <div className="text-center py-16 text-gray-400 text-sm">No thrift products found.</div>
          ) : (
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="col-span-2 space-y-1">
                ...
                <Input value={form.brand} onChange={(e) => setFormField('brand', e.target.value)}
                  placeholder="e.g. Levi's, Nike" className="h-9 text-sm" />
              </div>
              ...
              <div className="col-span-2 space-y-1">
                ...
                <Textarea
                  ...
                  placeholder="Describe the thrift item..."
      ...
      <Dialog open={deleteDialogOpen} onOpenChange={setDeleteDialogOpen}>
        <DialogContent className="max-w-sm">
          <DialogHeader>
            <DialogTitle>Delete Thrift Product?</DialogTitle>
          </DialogHeader>
          <p className="text-sm text-gray-500">This action cannot be undone. The product will be permanently removed from the thrift store.</p>
