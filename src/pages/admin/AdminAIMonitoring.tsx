...
const StatCard: React.FC<StatCardProps> = ({ label, value, icon: Icon, sub, highlight }) => (
  <div className={`bg-white border rounded-lg p-5 ${highlight ? 'border-blue-200' : 'border-gray-200'}`}>
    <div className="flex items-center justify-between mb-2">
      ...
      <Icon className={`h-4 w-4 ${highlight ? 'text-blue-500' : 'text-gray-400'}`} />
    </div>
    ...
    <AdminLayout>
      <div className="p-6 space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold text-gray-900">AI Monitoring</h1>
            <p className="text-sm text-gray-500 mt-0.5">FitverseAI try-on usage, performance, and errors</p>
          </div>
          <div className="flex items-center gap-3">
            {/* Maintenance toggle */}
            <div className="flex items-center gap-2 bg-white border border-gray-200 rounded-lg px-3 py-2">
              <WifiOff className={`h-4 w-4 ${stats.isMaintenanceMode ? 'text-red-500' : 'text-gray-300'}`} />
        ...
        {isLoading ? (
          ...
          <>
            ...
            <div className="bg-white border border-gray-200 rounded-lg p-5">
              <h2 className="text-sm font-semibold text-gray-700 mb-4">Daily Try-On Requests (Last 7 days)</h2>
            ...
            <div className="bg-white border border-gray-200 rounded-lg">
              <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
                <h2 className="text-sm font-semibold text-gray-700">Recent Error Log</h2>
                <span className="text-xs text-gray-400">{stats.errorCount} total errors</span>
              </div>
                  ...
                  {stats.recentErrors.map((err, i) => (
                    <div key={i} className="flex items-start gap-3 px-5 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {err.userId && <p className="text-xs text-gray-400 mt-0.5">User: {err.userId}</p>}
                      </div>
